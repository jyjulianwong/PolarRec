...
class S2rAdapter:
    ...
    """
    # A simplified version of the code used in the S2AGAdapter.
    ...
    @classmethod
    def _get_request_url_str(cls, resource, max_results_returned):
        """
        :param resource: The target resource.
        :type resource: Resource
        :param max_results_returned: Max. number of results returned by query.
        :type max_results_returned: int
        :return: The query component of the API request URL.
        :rtype: str
        """
        param_str = f"fields=paperId,title,authors&limit={max_results_returned}"
    ...
    @classmethod
    def _get_request_data(cls, resource, max_results_returned):
        """
        :param resource: The target resource.
        :type resource: Resource
        :param max_results_returned: Max. number of results returned by query.
        ...
        try:
            res = requests.get(
                cls._get_request_url_str(resource, max_results_returned),
                headers=headers,
                timeout=10
            )
    ...
    @classmethod
    def get_recommended_resources(cls, target_resource, max_results_returned):
        """
        Returns the list of recommendations provided by the S2R API.
        ...
        res = cls._get_request_data(target_resource, max_results_returned)
...
def get_recommend_accuracy(target_resource, keyword_model):
    ...
    :rtype: float
    """
    # The max. # of top results to use for comparison from both systems.
    max_compared_results = 30

    hit_count = 0
    miss_count = 0

    # The "gold standard" recommendation results provided by S2R.
    y_true = S2rAdapter.get_recommended_resources(
        target_resource=target_resource,
        max_results_returned=max_compared_results
    )
    # The results generated by this recommender system, i.e. the "predictions".
    ...
    # Make the lengths of two lists equal by considering the shorter list.
    max_compared_results = min(len(y_true), len(y_pred))
    y_true = y_true[:max_compared_results]
    y_pred = y_pred[:max_compared_results]

    # Use the titles of each resource as the resource's ID.
...
if __name__ == "__main__":
    # Run this .py file to calculate the system's recommendation accuracy.
